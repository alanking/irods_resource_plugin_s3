    ...
    irods::plugin_property_map& _prop_map)
{
    ...
    std::string resource_name = get_resource_name(_prop_map);
    // TODO: why do we not return immediately? key_id and acccess_key are not referenced
    // again, and there is no failure in the event that either key is missing from the
    // environment.
...
irods::error s3InitPerOperation (
    ...
    while( ctr < retry_count ) {
        ...
        if (status == S3StatusOK) {
            ...
            // TODO: This will never be true as status is not set in this compound conditional
            // statement whose predicate is status == S3StatusOK.
            if (status != S3StatusOK) {
