...
class Test_S3_Cache_Glacier_Base(session.make_sessions_mixin([('otherrods', 'rods')], [('alice', 'apass'), ('bobby', 'bpass')])):
    ...
    def test_iget_data_object_as_user_with_read_only_access_and_replica_only_in_archive__issue_6697(self):
        ...
        try:
            ...
            lib.delayAssert(
                lambda: readonly_user.assert_icommand(['iget', logical_path, '-'], 'STDOUT', contents),
                interval=20,
                maxrep=10)
    ...
    def test_iget_data_object_as_user_with_read_only_access_and_replica_only_in_archive__issue_6697(self):
        ...
        try:
            ...
            # Ensure that the read-only user has read-only permission on the data object.
            self.assert_permissions_on_data_object_for_user(writeonly_user.username, logical_path, 1050)
            ...
            # overwrite it), but the act of requesting the object has triggered a staging operation which should result
            ...
            # be done in less than 5).
            lib.delayAssert(
                lambda: writeonly_user.assert_icommand(['istream', 'write', '--append', logical_path], input=contents),
                interval=20,
                maxrep=10)
